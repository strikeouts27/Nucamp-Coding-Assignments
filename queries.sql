...
SELECT product_name
FROM products
WHERE units_in_stock <= reorder_level and units_on_order > 0 and discontinued = false
ORDER BY product_id;
-- I can make as many and statements as needed. 
-- if you do not know what logic to use: look at the column data type. 
-- numbers <= >= and booleans would be an evaluation of true or false, and than strings.... well it depends. 
...
-- you can use multiple statements for order by and even have instructions for how it is grouped.
-- as keyword- The AS command is used to rename a column or table with an alias.
...
-- SELECT ship address COUNT(column_name)
SELECT ship_address, COUNT(order_id) AS order_count
FROM orders
GROUP BY ship_address
ORDER BY order_count DESC
LIMIT 1;
-- Count(*) creates another row in our output answer with an order count column which contains the computers calculation. 
...
--SELECT does not require and statments. 
...
/*
WITH is creating a temporary table. It creates one column called shipper count. 
-- Run this first to see example presented. 
...
if we add the with statement line and the from statement line (lines 161 and 166) */ 

-- the second select is about getting the shipper count data averaged togather. 
...
SELECT p.product_name, c.category_name -- get this column from this table. -- if i am aliasing a column I use the AS keyword. c.category_name AS category. 
FROM products AS p -- alias the table here. alias tables by using the table name and than the AS alias name.
JOIN categories AS c -- creates the alias for the table here and joins it with products table.
ON c.category_id = p.category_id;

/* after join put the table name  and the alias of the table it belongs to. 
than the column name that you want to join and set and equals sign than select the other column name you want to select.
*/
...
FROM employees AS e -- aliasing the table employees table as e here. 
JOIN employees_territories AS et -- aliasing the employees territories table as et here. also saying we are joining the et table and the e table here.
ON e.employee_id = et.employee_id --combine the employee id columns here. 
JOIN territories AS t -- aliasing the territories table as t here. and we are setting the territories table to be combined with the employee_territories column. 
ON et.territory_id = t.territory_id -- combining the employees_territories table with the territory id column. 
JOIN regions AS r -- we use the JOIN function on region table.
ON r.region_id = t.region_id  -- the on command is going to ask us what part of the region table we are going to connect so we give it a column. And than we give it the other column from the other table. be certain to use the alias for that other table. 
ORDER BY r.region_description, t.territory_description,e.last_name,e.first_name;

-- we put the table name in front of the column name. or its alias. 
-- than the column name that you want to join and set and equals sign than select the other column name you want to select.
-- 3.3
...
--objective: find the territory description and region description. 
...
SELECT t.territory_description, r.region_description -- get me these values.
FROM territories AS t -- from the territories table
INNER JOIN regions AS r -- plus the regions table
...
-- where we have a territory that does not show up in the employees territories table. 
-- where are the empty spots in the employees territories table. 
